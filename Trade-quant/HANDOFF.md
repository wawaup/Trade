...
> 下方 §1 的策略画像已**作废**。四因子主动层被判定为 2022 单次熊市过拟合
> （OOS 2023+ 仅 +19.6%/0.59/−72.4%，全面输给月度等权持有），已封存。
> 当前冻结的可执行策略 = **月度等权持有该池 + vol_target 0.25**
> （全样本 +20.0%/0.86/−27.1%）。主动层重启门槛见 §15.6。
> 云端 paper 实验因旧代码全仓 bug 作废，待新策略实现后干净重启。
