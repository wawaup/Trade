...
> **⚠️ 2026-07-27 重大更新（RESEARCH_LOG §15）**：专业审核 + 真 walk-forward 之后，
> 下方 §1 的旧策略画像已**作废**（v1 被判定为 2022 单次熊市过拟合，OOS 2023+
> 仅 +19.6%/0.59/−72.4%，已封存，详见 §15.2/15.3）。
>
> **当前生产策略 = v2（§15.7/15.7b 冻结，live 已实现，136 测试全过）**：
> 四因子等权 z(Mom_12_1/Prox_52W/LowVol_60/RS_Beta) + Top-20 + 单票 10% +
> 无绝对过滤 + bear_flat_45 + 熔断−30% + VT0.25 + 10 日调仓
> → 全样本 +29.5%/1.29/−21.8%；OOS +46.9%/1.76/−15.1%；零熔断。
> 参数单源 `research/strategy_params.py` 的 `V2_*`；保底备选=月度等权+VT0.25。
> 云端 paper 实验因旧代码全仓 bug 作废；**下一步：按 `live/deploy/MOC_SWITCH_RUNBOOK.md`
> 部署 VM + paper 干净重启 + 3 个月参数冻结**（.env 注意 MAX_POSITION_PCT=0.10、
> SIM_CAPITAL_USD 留空）。
