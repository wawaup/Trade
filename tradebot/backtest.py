...
def run_backtest(
    ...
    for idx in range(2, len(intraday) + 1):
        ...
        if signal.action == "BUY" and cash >= strategy_config.min_order_quote and buy_is_spaced:
            quote = min(strategy_config.min_order_quote, cash, max_t_bucket - t_value)
            if quote >= strategy_config.min_order_quote:
        ...
        elif signal.action == "SELL" and t_qty > 0:
            fill_price = current.close * (1 - backtest_config.slippage_bps / 10_000)
